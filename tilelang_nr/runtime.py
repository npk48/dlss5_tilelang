"""Complete-NR plan reuse with explicitly replaced TileLang computation.

The native modules are prepared as immutable reference-plan metadata only: the
packet/layout plan, launch geometry and buffer ownership are reused, while every
logical Step is replaced by its TileLang implementation. No original math Step
may execute.
"""
...
from native_nr.runtime import NativeNR as CUDAReferenceNR
from native_nr.source import DEEP_UNITS
...
def outer_source(name):
    ...
    if name in ("clear_counters", "pool8_padding"):
        return "utility.cu"
    raise NotImplementedError("Unknown reference outer entry: " + name)


class TileLangNR(CUDAReferenceNR):
    ...
    def _prepare(self, key, features):
        ...
        memory = BufferViews(outer.driver.buffers + deep.d.buffers)
        memory.bind_input(features)
        names = {
            fn.value: name
            for d in (outer.driver, deep.d)
            for (_, name), fn in d.functions.items()
        }
        outer_defines = {"NR_H": outer.shape.height, "NR_W": outer.shape.width}
        records = []

        def outer_spec(step):
            name = names[step.function.value]
            return StepSpec(
                name, outer_source(name), step.geometry, step.values, outer_defines, memory
            )

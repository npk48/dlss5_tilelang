...
from .color import ColorSettings,decode_input,linear_to_srgb
from .post import OutputSettings
from .guides import GuideConfig
from .nr_chain import FrameCancelled,check_cancel
...
def run_manifest(manifest,output,*,device='cuda',weights=None,model=None,cancel=None,progress=None,before_frame=None,quiet=False):
    ...
    if spec.get('video'):
        ...
        from .media import probe_video,VideoFrames
    ...
    try:
        for index,f in enumerate(source_frames):
            ...
            if spec.get('video_output'):
                if writer is None:
                    from .media import PreviewVideo
            ...
            record={**result['info'],'input_index':index,'file':filename,'data_file':data_file,
                    'png_is_sdr_preview':hdr,'output_alpha_mode':out_alpha_mode,'seconds':time.perf_counter()-tick};records.append(record)
            processed=index+1
            ...
            report={'status':'running','frames':records,'processed_frames':processed,'frame_records':'frames.jsonl','seconds':time.perf_counter()-start,'source_manifest':str(manifest.resolve()),
                    'all_image_compute':'PyTorch; fixed NR; no native reference fallback','mode':mode,'guide_provenance':provenance,'video_source':video_info,

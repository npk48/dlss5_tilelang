...
extern "C" __global__ void __launch_bounds__(128, 1) main_kernel(const uchar* __restrict__ A, int* __restrict__ C0, uchar* __restrict__ Raw, const uchar* __restrict__ Weight) {
  ...
  for (int p = 0; p < 8; ++p) {
    for (int u = 0; u < 8; ++u) {
      uint condval;
      if ((((((int)blockIdx.x) * 2) + ((((u * 128) + ((int)threadIdx.x)) >> 8) & 1)) < 5)) {
        condval = nr_ld32((&(A[(((((((((((p * 20480) + (((((u * 128) + ((int)threadIdx.x)) >> 7) & 1) * 10240)) + (((((u * 128) + ((int)threadIdx.x)) >> 1) & 1) * 5120)) + (((int)blockIdx.y) * 2560)) + (((((u * 128) + ((int)threadIdx.x)) >> 9) & 1) * 1280)) + ((((u * 128) + ((int)threadIdx.x)) & 1) * 640)) + (((((u * 128) + ((int)threadIdx.x)) >> 6) & 1) * 320)) + (((int)blockIdx.x) * 128)) + (((((u * 128) + ((int)threadIdx.x)) >> 8) & 1) * 64)) + (((((u * 128) + ((int)threadIdx.x)) >> 4) & 3) * 16)) + ((((((int)threadIdx.x) & 31) >> 2) & 3) * 4))])));
      } else {
        condval = (uint)0;
      }
  ...
  __syncthreads();
  for (int t = 0; t < 4; ++t) {
    if (((((int)blockIdx.x) * 2) + (t & 1)) < 5) {
      for (int z_3 = 0; z_3 < 8; ++z_3) {
        uint v = nr_ld32((&(((uchar*)packed)[((((((((int)threadIdx.x) >> 5) * 4096) + (t * 1024)) + ((z_3 >> 2) * 512)) + ((((int)threadIdx.x) & 31) * 16)) + ((z_3 & 3) * 4))])));
        nr_st32((&(Raw[(((((((((((int)blockIdx.y) * 81920) + ((t >> 1) * 40960)) + (((int)blockIdx.x) * 16384)) + ((t & 1) * 8192)) + (((int)blockIdx.z) * 4096)) + (((((int)threadIdx.x) >> 5) & 3) * 1024)) + ((z_3 >> 2) * 512)) + ((((int)threadIdx.x) & 31) * 16)) + ((z_3 & 3) * 4))])), v);
      }
    }
  }

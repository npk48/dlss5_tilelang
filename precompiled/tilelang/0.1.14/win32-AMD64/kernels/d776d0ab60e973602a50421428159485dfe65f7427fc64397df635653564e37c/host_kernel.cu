...
extern "C" __global__ void main_kernel(const half_t* __restrict__ AH, const half_t* __restrict__ BH, int* __restrict__ C0, int* __restrict__ C1, const int* __restrict__ M, half_t* __restrict__ OH, int count, int n0, int n1, int sizes_0, int sizes_1, int sizes_2, int sizes_3, int v0, int v1);
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(const half_t* __restrict__ AH, const half_t* __restrict__ BH, int* __restrict__ C0, int* __restrict__ C1, const int* __restrict__ M, half_t* __restrict__ OH, int count, int n0, int n1, int sizes_0, int sizes_1, int sizes_2, int sizes_3, int v0, int v1) {
  if (((((int)blockIdx.x) * 256) + ((int)threadIdx.x)) < count) {
    ...
    half_t h = ((half_t)nr_add(((float)condval), ((float)condval_5)));
    if (((((int)blockIdx.x) * 256) + ((int)threadIdx.x)) < (sizes_3 >> 1)) {
      OH[((((int64_t)((int)blockIdx.x)) * (int64_t)256) + ((int64_t)((int)threadIdx.x)))] = h;
    }
  }
  if (((int)blockIdx.x) == 0) {

...
extern "C" __global__ void main_kernel(const half_t* __restrict__ AH, const int* __restrict__ M, half_t* __restrict__ OH);
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(const half_t* __restrict__ AH, const int* __restrict__ M, half_t* __restrict__ OH) {
  half_t h = half_t(0x0p+0f/*0.000000e+00*/);
  h = AH[((((int)blockIdx.x) * 256) + ((int)threadIdx.x))];
  for (int p = 1; p < 3; ++p) {
    half_t a = h;
    half_t b = AH[(((p * 98304) + (((int)blockIdx.x) * 256)) + ((int)threadIdx.x))];
    h = ((half_t)nr_add(((float)a), ((float)b)));
  }
  if (0 <= M[((((int)blockIdx.x) * 256) + ((int)threadIdx.x))]) {
    if (M[((((int)blockIdx.x) * 256) + ((int)threadIdx.x))] < 393216) {
      OH[M[((((int64_t)((int)blockIdx.x)) * (int64_t)256) + ((int64_t)((int)threadIdx.x)))]] = h;
    }
  }
}

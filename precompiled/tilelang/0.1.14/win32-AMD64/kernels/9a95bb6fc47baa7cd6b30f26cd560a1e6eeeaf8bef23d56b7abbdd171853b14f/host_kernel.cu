  ...
  for (int slab = 0; slab < 2; ++slab) {
    ...
    for (int parity = 0; parity < 2; ++parity) {
      ...
      *(uint4*)(temp + 0) = make_uint4(broadcast_var, broadcast_var, broadcast_var, broadcast_var);
      if ((0 <= raw) & (raw <= 18578832)) {
        {
          int SW_local_cast[4];
          *(int4*)(SW_local_cast + 0) = *(int4*)(SW + (((raw >> 4) * 4) + 10444800));
          uint4 __1;
      ...
      if ((0 <= raw_1) & (raw_1 <= 17923472)) {
        {
          ...
          *(int4*)(RW + (((raw_1 >> 4) * 4) + 10608640)) = *(int4*)(RW_local_cast_1 + 0);
        }
      }
    }
    __syncthreads();

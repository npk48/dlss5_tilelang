...
extern "C" __global__ void main_kernel(const half_t* __restrict__ AH, const uchar* __restrict__ B, int* __restrict__ C0, int* __restrict__ C1, const half_t* __restrict__ G, const int* __restrict__ M, uchar* __restrict__ O, const int* __restrict__ P);
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(const half_t* __restrict__ AH, const uchar* __restrict__ B, int* __restrict__ C0, int* __restrict__ C1, const half_t* __restrict__ G, const int* __restrict__ M, uchar* __restrict__ O, const int* __restrict__ P) {
  int j = M[((((int)blockIdx.x) * 256) + ((int)threadIdx.x))];
  uchar condval;
  if (((0 <= j) && (j < 163840))) {
    condval = B[((int64_t)j)];
  } else {
  ...
  uchar a = condval;
  half_t a_1 = ((half_t)nr_dec(a));
  half_t b = G[(j & 511)];
  half_t condval_1;
  if (((0 <= j) && (j < 196608))) {
    condval_1 = AH[((((j >> 14) * 4096) + (((j & 8191) >> 10) * 512)) + (j & 511))];
  } else {
    condval_1 = half_t(0x0p+0f/*0.000000e+00*/);
  }
  half_t c = condval_1;
  half_t a_2 = ((half_t)nr_fma(((float)a_1), ((float)b), ((float)c)));
  if (0 <= P[((((int)blockIdx.x) * 256) + ((int)threadIdx.x))]) {
    if (P[((((int)blockIdx.x) * 256) + ((int)threadIdx.x))] < 163840) {
      O[P[((((int64_t)((int)blockIdx.x)) * (int64_t)256) + ((int64_t)((int)threadIdx.x)))]] = nr_enc(((float)a_2));
    }
  }
  if (((int)blockIdx.x) == 0) {

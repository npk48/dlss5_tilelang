...
extern "C" __global__ void main_kernel(const half_t* __restrict__ AH, int* __restrict__ C0, int* __restrict__ C1, const int* __restrict__ M, half_t* __restrict__ OH, int count, int n0, int n1, int sizes_0, int sizes_2, int sizes_3, int v0, int v1);
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(const half_t* __restrict__ AH, int* __restrict__ C0, int* __restrict__ C1, const int* __restrict__ M, half_t* __restrict__ OH, int count, int n0, int n1, int sizes_0, int sizes_2, int sizes_3, int v0, int v1) {
  half_t h = half_t(0x0p+0f/*0.000000e+00*/);
  if (((((int)blockIdx.x) * 256) + ((int)threadIdx.x)) < count) {
    half_t condval;
    if ((((((int)blockIdx.x) * 256) + ((int)threadIdx.x)) < (sizes_0 >> 1))) {
      condval = AH[((((int64_t)((int)blockIdx.x)) * (int64_t)256) + ((int64_t)((int)threadIdx.x)))];
    } else {
    ...
    for (int p = 1; p < 3; ++p) {
      half_t a = h;
      half_t condval_1;
      if (((0 <= (((((int)blockIdx.x) * 256) + (p * count)) + ((int)threadIdx.x))) && ((((((int)blockIdx.x) * 256) + (p * count)) + ((int)threadIdx.x)) < (sizes_0 >> 1)))) {
        condval_1 = AH[(((((int64_t)((int)blockIdx.x)) * (int64_t)256) + (((int64_t)p) * ((int64_t)count))) + ((int64_t)((int)threadIdx.x)))];
      } else {
        condval_1 = half_t(0x0p+0f/*0.000000e+00*/);
      }
      half_t b = condval_1;
      h = ((half_t)nr_add(((float)a), ((float)b)));
    }
    int condval_2;

  ...
  for (int slab = 0; slab < 2; ++slab) {
    ...
    for (int parity = 0; parity < 2; ++parity) {
      ...
      *(uint4*)(temp + 0) = make_uint4(broadcast_var, broadcast_var, broadcast_var, broadcast_var);
      if ((0 <= raw) & (raw <= 27590032)) {
        {
          int SW_local_cast[4];
          *(int4*)(SW_local_cast + 0) = *(int4*)(SW + (((raw >> 4) * 4) + 8192000));
          uint4 __1;
      ...
      if ((0 <= raw_1) & (raw_1 <= 26934672)) {
        {
          ...
          *(int4*)(RW + (((raw_1 >> 4) * 4) + 8355840)) = *(int4*)(RW_local_cast_1 + 0);
        }
      }
    }
    __syncthreads();

...
extern "C" __global__ void __launch_bounds__(128, 1) main_kernel(uchar* __restrict__ A, const half_t* __restrict__ G, uint* __restrict__ P, uchar* __restrict__ S, uchar* __restrict__ W) {
  ...
  if (((int)blockIdx.z) == 0) {
    ...
    for (int r_1 = 0; r_1 < 4; ++r_1) {
      ...
      for (int g = 0; g < 2; ++g) {
        ...
        #pragma unroll
        for (int j = 0; j < 4; ++j) {
          half_t a = G[((((((((int)blockIdx.y) * 128) + (((((int)threadIdx.x) & 63) >> 5) * 64)) + (g * 32)) + (j * 8)) + (((((int)threadIdx.x) & 31) & 3) * 2)) + 524288)];
          half_t a_1 = G[((((((((int)blockIdx.y) * 128) + (((((int)threadIdx.x) & 63) >> 5) * 64)) + (g * 32)) + (j * 8)) + (((((int)threadIdx.x) & 31) & 3) * 2)) + 524289)];
  ...
  for (int h_2 = 0; h_2 < 2; ++h_2) {
    nr_tl_vit_joint::cp((&(sh[((h_2 * 2048) + (((int)threadIdx.x) * 16))])), (&(A[0])), ((((h_2 * 65536) + ((((int)threadIdx.x) >> 5) * 16384)) + (((int)blockIdx.z) * 4096)) + ((((int)threadIdx.x) & 31) * 16)), ((int)(((h_2 * 2) + (((int)threadIdx.x) >> 6)) < 3)));
  }
  #pragma unroll
  ...
  for (int phase = 0; phase < 8; ++phase) {
    ...
    if (phase < 7) {
      ...
      for (int h_3 = 0; h_3 < 2; ++h_3) {
        nr_tl_vit_joint::cp((&(sh[(((((phase + 1) & 1) * 8192) + (h_3 * 2048)) + (((int)threadIdx.x) * 16))])), (&(A[0])), ((((((h_3 * 65536) + ((((int)threadIdx.x) >> 5) * 16384)) + (((int)blockIdx.z) * 4096)) + (phase * 512)) + ((((int)threadIdx.x) & 31) * 16)) + 512), ((int)(((h_3 * 2) + (((int)threadIdx.x) >> 6)) < 3)));
      }
      #pragma unroll

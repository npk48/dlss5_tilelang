  ...
  for (int n = 0; n < 4; ++n) {
    ...
    for (int kp = 0; kp < 4; ++kp) {
      ...
      for (int word = 0; word < 4; ++word) {
        a[word] = (uint)0;
        int base = IM[(((((((int)blockIdx.x) * 2048) + ((word & 1) * 1024)) + ((((int)threadIdx.x) >> 2) * 128)) + (kp * 32)) + ((word >> 1) * 16))];
        if (0 <= base) {
          int condval;
          if ((((base >> 2) + (((int)threadIdx.x) & 3)) < 4153192)) {
            condval = R[(((base >> 2) + (((int)threadIdx.x) & 3)) + 10936320)];
          } else {
            condval = 0;
          }
      ...
      uint64_t r = wide_mma(a[0], a[1], a[2], a[3], W[(((((kp * 512) + (((int)blockIdx.y) * 256)) + ((n >> 1) * 128)) + (((int)threadIdx.x) * 4)) + ((n & 1) * 2))], W[((((((kp * 512) + (((int)blockIdx.y) * 256)) + ((n >> 1) * 128)) + (((int)threadIdx.x) * 4)) + ((n & 1) * 2)) + 1)], acc[0], acc[1]);
      acc[0] = ((uint)r);
    ...
    for (int j = 0; j < 2; ++j) {
      uint v_ = acc[j];
      Proj[((((((((int)blockIdx.x) * 512) + (j * 256)) + ((((int)threadIdx.x) >> 2) * 32)) + (((int)blockIdx.y) * 16)) + (n * 4)) + (((int)threadIdx.x) & 3))] = (*(int *)(&(v_)));
    }
  }
}

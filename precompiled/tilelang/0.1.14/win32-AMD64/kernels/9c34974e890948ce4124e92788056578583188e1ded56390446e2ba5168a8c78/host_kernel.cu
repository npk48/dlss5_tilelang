...
extern "C" __global__ void main_kernel(const half_t* __restrict__ AH, const half_t* __restrict__ BH, const int* __restrict__ M, half_t* __restrict__ OH);
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(const half_t* __restrict__ AH, const half_t* __restrict__ BH, const int* __restrict__ M, half_t* __restrict__ OH) {
  half_t condval;
  ...
  half_t a = condval;
  half_t b = AH[(((((int)blockIdx.x) * 256) + ((int)threadIdx.x)) + 147456)];
  half_t h = ((half_t)nr_add(((float)a), ((float)b)));
  OH[((((int)blockIdx.x) * 256) + ((int)threadIdx.x))] = h;
}

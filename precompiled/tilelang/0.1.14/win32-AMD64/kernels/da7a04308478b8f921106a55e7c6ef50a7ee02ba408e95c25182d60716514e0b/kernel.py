...
def call(kernels, C, DD, Rec, DM, MV, React, Comp, VM, DI, CN, P, Prepared, Masks, stream=0):
    
    res = cuKernelSetAttribute(
        CUfunction_attribute.CU_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES,
        0,
        kernels["main_kernel"],
        CUdevice(C.device.index)
    )[0]
    ...
    arg_values = C.data_ptr(), CN.data_ptr(), Comp.data_ptr(), DD.data_ptr(), DI.data_ptr(), DM.data_ptr(), MV.data_ptr(), Masks.data_ptr(), P.data_ptr(), Prepared.data_ptr(), React.data_ptr(), Rec.data_ptr(), VM.data_ptr()

...
extern "C" __global__ void main_kernel(const uchar* __restrict__ A, const int* __restrict__ M, uchar* __restrict__ O, const int* __restrict__ P);
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(const uchar* __restrict__ A, const int* __restrict__ M, uchar* __restrict__ O, const int* __restrict__ P) {
  if (0 <= P[((((int)blockIdx.x) * 256) + ((int)threadIdx.x))]) {
    if (P[((((int)blockIdx.x) * 256) + ((int)threadIdx.x))] < 131072) {
      ...
      O[P[((((int64_t)((int)blockIdx.x)) * (int64_t)256) + ((int64_t)((int)threadIdx.x)))]] = condval;
    }
  }
}

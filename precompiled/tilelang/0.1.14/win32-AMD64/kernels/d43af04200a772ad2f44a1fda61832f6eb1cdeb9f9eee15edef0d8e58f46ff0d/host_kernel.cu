...
extern "C" __global__ void kernel_kernel(const int* __restrict__ IM, int* __restrict__ Proj, const int* __restrict__ R, const int* __restrict__ W);
extern "C" __global__ void __launch_bounds__(32, 1) kernel_kernel(const int* __restrict__ IM, int* __restrict__ Proj, const int* __restrict__ R, const int* __restrict__ W) {
  uint acc[2];
  uint a[4];
  for (int n = 0; n < 4; ++n) {
    acc[0] = (uint)0;
    ...
    for (int kp = 0; kp < 8; ++kp) {
      ...
      for (int word = 0; word < 4; ++word) {
        a[word] = (uint)0;
        int base = IM[((((((int)blockIdx.x) * 4096) + ((word & 1) * 2048)) + ((((int)threadIdx.x) >> 2) * 256)) + (((((((kp * 32) + ((word >> 1) * 16)) + ((((int)threadIdx.x) & 3) * 4)) & -15) | (((((kp * 32) + ((word >> 1) * 16)) + ((((int)threadIdx.x) & 3) * 4)) & 2) << 2)) | (((((kp * 32) + ((word >> 1) * 16)) + ((((int)threadIdx.x) & 3) * 4)) & 4) >> 1)) | (((((kp * 32) + ((word >> 1) * 16)) + ((((int)threadIdx.x) & 3) * 4)) & 8) >> 1)))];
        if (0 <= base) {
          int condval;
          if ((base < 20217248)) {
            condval = R[((base >> 2) + 10035200)];
          } else {
            condval = 0;
          }
          int _reinterpret_tmp = condval;
          a[word] = (*(uint *)(&(_reinterpret_tmp)));
        }
      }
      uint64_t r = wide_mma(a[0], a[1], a[2], a[3], W[((((((kp * 1024) + (((int)blockIdx.y) * 256)) + ((n >> 1) * 128)) + (((int)threadIdx.x) * 4)) + ((n & 1) * 2)) + 24576)], W[((((((kp * 1024) + (((int)blockIdx.y) * 256)) + ((n >> 1) * 128)) + (((int)threadIdx.x) * 4)) + ((n & 1) * 2)) + 24577)], acc[0], acc[1]);
      acc[0] = ((uint)r);
    ...
    for (int j = 0; j < 2; ++j) {
      uint v_ = acc[j];
      Proj[((((((((int)blockIdx.x) * 1024) + (j * 512)) + ((((int)threadIdx.x) >> 2) * 64)) + (((int)blockIdx.y) * 16)) + (n * 4)) + (((int)threadIdx.x) & 3))] = (*(int *)(&(v_)));
    }
  }
}

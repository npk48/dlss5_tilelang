...
extern "C" __global__ void main_kernel(const half_t* __restrict__ AH, const half_t* __restrict__ BH, int* __restrict__ C0, int* __restrict__ C1, const int* __restrict__ M, uchar* __restrict__ O, const int* __restrict__ P);
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(const half_t* __restrict__ AH, const half_t* __restrict__ BH, int* __restrict__ C0, int* __restrict__ C1, const int* __restrict__ M, uchar* __restrict__ O, const int* __restrict__ P) {
  half_t condval;
  if (((0 <= M[((((int)blockIdx.x) * 256) + ((int)threadIdx.x))]) && (M[((((int)blockIdx.x) * 256) + ((int)threadIdx.x))] < 393216))) {
    condval = BH[M[((((int64_t)((int)blockIdx.x)) * (int64_t)256) + ((int64_t)((int)threadIdx.x)))]];
  } else {
    condval = half_t(0x0p+0f/*0.000000e+00*/);
  }
  half_t a = condval;
  half_t b = AH[(((((int)blockIdx.x) * 256) + ((int)threadIdx.x)) + 294912)];
  half_t a_1 = ((half_t)nr_add(((float)a), ((float)b)));
  half_t a_2 = cutlass::half_t(__hmax((cutlass::half_t(__hmin((a_1).to_half(), (half_t(0x1p+2f/*4.000000e+00*/)).to_half()))).to_half(), (half_t(-0x1p+2f/*-4.000000e+00*/)).to_half()));
  half_t a_3 = __habs(cutlass::half_t(__hmax((cutlass::half_t(__hmin((a_1).to_half(), (half_t(0x1p+2f/*4.000000e+00*/)).to_half()))).to_half(), (half_t(-0x1p+2f/*-4.000000e+00*/)).to_half())));
  half_t b_1 = ((half_t)nr_fma(((float)__habs(cutlass::half_t(__hmax((cutlass::half_t(__hmin((a_1).to_half(), (half_t(0x1p+2f/*4.000000e+00*/)).to_half()))).to_half(), (half_t(-0x1p+2f/*-4.000000e+00*/)).to_half())))), -0x1.cap-5f/*-5.590820e-02*/, 0x1.cap-2f/*4.472656e-01*/));
  half_t b_2 = ((half_t)nr_fma(((float)cutlass::half_t(__hmax((cutlass::half_t(__hmin((a_1).to_half(), (half_t(0x1p+2f/*4.000000e+00*/)).to_half()))).to_half(), (half_t(-0x1p+2f/*-4.000000e+00*/)).to_half()))), ((float)b_1), 0x1.cap-1f/*8.945312e-01*/));
  if (0 <= P[((((int)blockIdx.x) * 256) + ((int)threadIdx.x))]) {
    if (P[((((int)blockIdx.x) * 256) + ((int)threadIdx.x))] < 131072) {
      O[P[((((int64_t)((int)blockIdx.x)) * (int64_t)256) + ((int64_t)((int)threadIdx.x)))]] = nr_enc(((float)a_1));
    }
  }
  if (((int)blockIdx.x) == 0) {
    if (((int)threadIdx.x) < 8) {
      C0[((int)threadIdx.x)] = 3;

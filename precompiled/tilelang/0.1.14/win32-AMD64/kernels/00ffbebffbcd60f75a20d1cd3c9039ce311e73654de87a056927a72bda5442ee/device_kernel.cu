...
extern "C" __global__ void main_kernel(int* __restrict__ C0, uchar* __restrict__ K, uchar* __restrict__ O, uchar* __restrict__ Q, uchar* __restrict__ V);
extern "C" __global__ void __launch_bounds__(128, 1) main_kernel(int* __restrict__ C0, uchar* __restrict__ K, uchar* __restrict__ O, uchar* __restrict__ Q, uchar* __restrict__ V) {
  uint query[16];
  uint result[32];
  half_t den[8];
  ...
  for (int r = 0; r < 4; ++r) {
    ...
    if ((((((int)threadIdx.x) >> 5) * 2) + (r >> 1)) < 3) {
      nr_tl_vit_joint::global4((&(query[(r * 4)])), (&(Q[(((((((int)threadIdx.x) >> 5) * 65536) + (r * 16384)) + (((int)blockIdx.x) * 512)) + ((((int)threadIdx.x) & 31) * 16))])));
    }
    #pragma unroll
    for (int n = 0; n < 4; ++n) {
      #pragma unroll
  ...
  nr_tl_vit_joint::cp((&(sh[(((int)threadIdx.x) * 16)])), (&(K[0])), ((((((int)threadIdx.x) >> 5) * 16384) + (((int)blockIdx.x) * 512)) + ((((int)threadIdx.x) & 31) * 16)), 1);
  ...
  for (int phase = 0; phase < 2; ++phase) {
    ...
    __syncthreads();
    if (phase < 1) {
      nr_tl_vit_joint::cp((&(sh[((((int)threadIdx.x) * 16) + 4096)])), (&(K[0])), (((((phase * 65536) + ((((int)threadIdx.x) >> 5) * 16384)) + (((int)blockIdx.x) * 512)) + ((((int)threadIdx.x) & 31) * 16)) + 65536), ((int)(((phase * 2) + (((int)threadIdx.x) >> 6)) < 1)));
      nr_tl_vit_joint::cp((&(sh[((((int)threadIdx.x) * 16) + 6144)])), (&(V[0])), (((((phase * 65536) + ((((int)threadIdx.x) >> 6) * 32768)) + (((int)blockIdx.x) * 1024)) + ((((int)threadIdx.x) & 63) * 16)) + 65536), 1);
      nr_tl_vit_joint::commit();
    }
    #pragma unroll
    ...
    for (int pair = 0; pair < 4; ++pair) {
      nr_tl_vit_joint::shared4((&(bv[0])), (&(sh[(((phase * 4096) + (pair * 512)) + ((((int)threadIdx.x) & 31) * 16))])));
      #pragma unroll
    ...
    for (int r_3 = 0; r_3 < 4; ++r_3) {
      ...
      for (int kp = 0; kp < 2; ++kp) {
        ...
        for (int n_6 = 0; n_6 < 2; ++n_6) {
          nr_tl_vit_joint::shared4((&(bv[(n_6 * 4)])), (&(sh[(((((phase * 4096) + (kp * 1024)) + (n_6 * 512)) + ((((int)threadIdx.x) & 31) * 16)) + 2048)])));
        }
        #pragma unroll
  ...
  for (int r_4 = 0; r_4 < 4; ++r_4) {
    ...
    for (int h_5 = 0; h_5 < 2; ++h_5) {
      ...
      half_t d = ((half_t)nr_add(((float)a_16), -0x1.58p+1f/*-2.687500e+00*/));
      ...
      for (int n_8 = 0; n_8 < 4; ++n_8) {
        ...
        result[(((r_4 * 8) + (n_8 * 2)) + h_5)] = nr_mul2(a_18, b_8);
      }
    }
    if ((((((int)threadIdx.x) >> 5) * 2) + (r_4 >> 1)) < 3) {
      #pragma unroll
      ...
      nr_tl_vit_joint::store4((&(O[(((((((int)threadIdx.x) >> 5) * 65536) + (r_4 * 16384)) + (((int)blockIdx.x) * 512)) + ((((int)threadIdx.x) & 31) * 16))])), packet[0], packet[1], packet[2], packet[3]);
    }
  }
  bool first_block = (((int)blockIdx.x) == 0);
  if (((int)blockIdx.x) == 0) {

...
function depthGrid(source: Source | null, size: number) {
  ...
  if (ratio > 1.78) size = evenRound(Math.trunc(size * 1.777 / ratio) / 14) * 14;
...
export default function App() {
  ...
  const requiredModels = [...(config.depth && grid ? [`vda_small_${grid}_init.onnx`, `vda_small_${grid}_step.onnx`] : []), ...(config.flow ? [`raft_small_u${config.flow_updates}.onnx`] : [])];
  ...
  const validation = invalidAxes(config.nr_width, config.nr_height) || invalidAxes(config.width, config.height) ? 'Set both axes to 0 (automatic), or both to integers between 32 and 16384.' : (config.guide || config.reconstruction) && (!config.depth || !config.flow) ? 'Guide and reconstruction need both depth and flow estimators for image uploads.' : config.depth && source && !grid ? 'This aspect ratio is too extreme for the selected VDA input size.' : missing.length ? `Required model files not reported by server: ${missing.join(', ')}` : '';

...
std::string encode_png(const Image& image){
 ...
 ComPtr<IWICBitmapFrameEncode> frame;ComPtr<IPropertyBag2> props;ok(encoder->CreateNewFrame(&frame,&props));ok(frame->Initialize(props.Get()));ok(frame->SetSize(image.width,image.height));WICPixelFormatGUID format=GUID_WICPixelFormat32bppRGBA;ok(frame->SetPixelFormat(&format));

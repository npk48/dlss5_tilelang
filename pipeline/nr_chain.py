...
@spatial_jit(dynamic='H W NH NW RH RW Cold StrengthBits InvHBits InvWBits',target=TARGET,execution_backend=EXECUTION_BACKEND,pass_configs=CONFIG,compile_flags=['--fmad=false'])
def prepare_kernel(H,W,NH,NW,RH,RW,Cold,StrengthBits,InvHBits,InvWBits):
 @T.prim_func
 def main(Work:T.Tensor((1,3,H,W),T.float32),Motion:T.Tensor((RH,RW,2),T.float32),Confidence:T.Tensor((H,W,1),T.float32),Current:T.Tensor((1,NH,NW,3),T.float32),MV:T.Tensor((1,NH,NW,2),T.float32),Gate:T.Tensor((1,NH,NW,1),T.float32)):
  with T.Kernel(T.ceildiv(NH*NW,128),threads=128) as bx:
   ...
   if index<NH*NW:
    ...
    if Cold == 0:
     if T.And(y<H,x<W):
      iy=T.min(T.cast(T.floor(((T.cast(y,T.float32)+T.float32(.5))*T.float32(RH))*T.reinterpret('float32',T.cast(InvHBits,'uint32'))),T.int32),RH-1)
      ix=T.min(T.cast(T.floor(((T.cast(x,T.float32)+T.float32(.5))*T.float32(RW))*T.reinterpret('float32',T.cast(InvWBits,'uint32'))),T.int32),RW-1)
      ...
      MV[0,y,x,0]=dx;MV[0,y,x,1]=dy;Gate[0,y,x,0]=(T.cast(visible,T.float32)*Confidence[y,x,0])*T.reinterpret('float32',T.cast(StrengthBits,'uint32'))
     else:MV[0,y,x,0]=T.float32(0);MV[0,y,x,1]=T.float32(0);Gate[0,y,x,0]=T.float32(0)
    else:MV[0,y,x,0]=T.float32(0);MV[0,y,x,1]=T.float32(0);Gate[0,y,x,0]=T.float32(0)
 return main

@spatial_jit(dynamic='H W',target=TARGET,execution_backend=EXECUTION_BACKEND,pass_configs=CONFIG,compile_flags=['--fmad=false'])
...
class Chain:
 ...
 def __call__(self,encoded,motion_uv,confidence,*,reset=False,cancel=None,resample='bilinear'):
  ...
  for index,settings in enumerate(c.passes):
   ...
   current=torch.empty((1,nh,nw,3),device=work.device,dtype=torch.float32)
   mv=torch.empty((1,nh,nw,2),device=work.device,dtype=torch.float32)
   gate=torch.empty((1,nh,nw,1),device=work.device,dtype=torch.float32)
   prepare_kernel(h,w,nh,nw,rh,rw,int(cold),float_bits(settings.temporal_strength),float_bits(recip(h)),float_bits(recip(w)))(work.contiguous(),motion_uv.contiguous(),conf,current,mv,gate)
   ...
   packet=torch.empty((1,16,nh,nw),device=work.device,dtype=torch.float32)
   packet_kernel(nh,nw)(sampled_current,sampled_previous,gaussian.contiguous(),gate,scalars,packet)
   head=c.model.infer_minimal(packet)
   result=torch.empty_like(sampled_current)
   output_kernel(nh,nw)(sampled_current,sampled_previous,head[...,:3].float().contiguous(),torch.sigmoid(head[...,3:4].float()).contiguous(),gate,result)
